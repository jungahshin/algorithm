...
void go(){
    ...
    for(int h=0; h<C; h++){//낚시왕 이동
        ...
        for(int i=0; i<v_location.size(); i++){//모든 상어에 적용
            // cout<<"A"<<v_location[i].first<<"/"<<v_location[i].second<<"/"<<a[v_location[i].first][v_location[i].second]<<"\n";
            if(a[v_location[i].first][v_location[i].second] != -1){//상어가 있을 시에 진행
                // cout<<"이동 전"<<v_location[i].first<<"/"<<v_location[i].second<<"/"<<i<<"\n";
                a[v_location[i].first][v_location[i].second] = -1;//원래 위치 0으로 만들고
                for(int j=0; j<(get<0>(v_else[i])); j++){//특정 상어의 속력만큼 반복
                ...
                // cout<<"이동 후"<<v_location[i].first<<"/"<<v_location[i].second<<"\n";
                if(temp[v_location[i].first][v_location[i].second] == -1){//새로 바뀐 위치에 아무것도 없으면
                    ...
                    // cout<<"상어 중복 위치"<<v_location[i].first<<"/"<<v_location[i].second<<"\n";
                    //현재 index-->i, 이미 있는 상어의 index --> a[v_location[i].first][v_location[i].second]
                    // cout<<"상어 크기"<<get<2>(v_else[i])<<"/"<<get<2>(v_else[(temp[v_location[i].first][v_location[i].second])])<<"\n";
                    if(get<2>(v_else[i]) > get<2>(v_else[(temp[v_location[i].first][v_location[i].second])])){
                        v_location[(temp[v_location[i].first][v_location[i].second])].first = -1;
                        v_location[(temp[v_location[i].first][v_location[i].second])].second = -1;
                        temp[v_location[i].first][v_location[i].second] = i;
                        // cout<<"index"<<i<<"\n";
                    }
                }

...
int main(){
    ...
    if(start == goal){
        ...
        for(int i=0; i<start_include.size(); i++){
            for(int j=0; j<goal_include.size(); j++){
                if(start_include[i].first == goal_include[j].first){//구역이 같으면
                    duplicate++;//중복의 개수를 센다.
                }
            }
        }


        if(duplicate == 0){//하나도 중복 값이 없으면 둘은 전혀 어디에도 포함되지 않는 다른 곳에 있으므로 0을 지나야 한다.
        ...
        }else if(duplicate == 1){//중복 값이 하나라도 있으면 두 구역은 어느 구역에 같이 포함되어 있는 것

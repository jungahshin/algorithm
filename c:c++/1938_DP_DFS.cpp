...
int go(int a, int b, int c, char d[][51], int num){//중간점을 항상 기준으로 생각한다. 큐에 있는 dir이 0이면-->(x, y-1),(x, y),(x, y+1), dir이 1이면 -->(x-1, y),(x, y),(x+1, y)
    ...
    if(c == 0){//행
        ...
        if(0 <= (a-1) && (a-1) < n){
            if((d[a-1][b-1] == '0' || d[a-1][b-1] == 'E') && (d[a-1][b] == '0' || d[a-1][b] == 'E') && (d[a-1][b+1] == '0' || d[a-1][b+1] == 'E') && !visited[a-1][b][0]){//모두 빈칸이면 이동 가능!
                ...
                d[a][b] = '0';
                d[a-1][b] = 'B';
                dp[a][b][c] = min(dp[a][b][c], go(a-1, b, 0, d, num+1)+1);
                visited[a-1][b][0] = 0;
                d[a][b] = 'B';
                d[a-1][b] = '0';
            }
        ...
        if(0 <= (b-2) && (b-2) < n){
            if((d[a][b-2] == '0' || d[a][b-2] == 'E') && !visited[a][b-1][0]){
                ...
                d[a][b] = '0';
                d[a][b-1] = 'B';
                dp[a][b][c] = min(dp[a][b][c], go(a, b-1, 0, d, num+1)+1);
                visited[a][b-1][0] = 0;
                d[a][b] = 'B';
                d[a][b-1] = '0';
            }
        ...
        if(temp == 8 && !visited[a][b][1]){//회전 가능
            visited[a][b][1] = 1;
            dp[a][b][c] = min(dp[a][b][c], go(a, b, 1, d, num+1)+1);
            visited[a][b][1] = 0;
        }
    }else if(c == 1){//열
       //U(상)-->x값들을 -1씩 한다.(x값들만 범위 내에 있는 지 체크)
        if(0 <= (a-2) && (a-2) < n){
            if((d[a-2][b] == '0' || d[a-2][b] == 'E') && !visited[a-1][b][1]){//모두 빈칸이면 이동 가능!
                ...
                d[a][b] = '0';
                d[a-1][b] = 'B';
                dp[a][b][c] = min(dp[a][b][c], go(a-1, b, 1, d, num+1)+1);
                visited[a-1][b][1] = 0;
                d[a][b] = 'B';
                d[a-1][b] = '0';
            }
        ...
        if(0 <= (b-1) && (b-1) < n){
            if((d[a-1][b-1] == '0' || d[a-1][b-1] == 'E') && (d[a][b-1] == '0' || d[a][b-1] == 'E') && (d[a+1][b-1] == '0' || d[a+1][b-1] == 'E') && !visited[a][b-1][1]){
                ...
                d[a][b] = '0';
                d[a][b-1] = 'B';
                dp[a][b][c] = min(dp[a][b][c], go(a, b-1, 1, d, num+1)+1);
                visited[a][b-1][1] = 0;
                d[a][b] = 'B';
                d[a][b-1] = '0';
            }
        ...
        if(temp == 8 && !visited[a][b][0]){//회전 가능
            visited[a][b][0] = 1;
            dp[a][b][c] = min(dp[a][b][c], go(a, b, 0, d, num+1)+1);
            visited[a][b][0] = 0;
        }
    }
    return dp[a][b][c];
...
int main(){
    ...
    if(start[0].second == start[1].second){//열의 값이 같으면 --> dir 1
        ...
        result = go(start[1].first, start[1].second, 1, save, 0);
    }else{
        visited[start[1].first][start[1].second][0] = 1;
        result = go(start[1].first, start[1].second, 0, save, 0);
    }
    
    if(final_num == false){//이동 불가
        cout<<"0"<<"\n";
    }else{
        cout<<result<<"\n";
    }
